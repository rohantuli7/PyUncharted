...
class Pyramid1():
    def __init__(self, pyramidStart, pyramidMap, pyramidDesc, bag):
        ...
        self.location = tk.Label(self.win, text = "Chapter 7 (Location : Pyramid)", bg='black', fg='White')
        ...
        self.task = tk.Label(self.win, text = "Task 1",fg='red')
    ...
    def item_selection(self, text):
        ...
        if variable == 1:
            ...
            self.selection.configure(text = "Incorrect item selected!",fg='red')
    ...
    def flame(self):
        ...
        for i, (text, value) in enumerate(values.items()):
            tk.Radiobutton(self.win, text=text, variable=self.var, value=value, bg='#567', fg='White', command = lambda: self.item_selection(text)).grid(row = 6, column = i+2)
    ...
    def courtyard(self):
        ...
        self.l1 = tk.Label(self.win, text="(On completion of this round, you will be directed to the next room!)",bg='black',fg='white')
        self.l2 = tk.Label(self.win, text="(Failure to find the correct items will lead to redirection to the same room!)",bg='black',fg='white')
        tk.Button(self.win, text="Quit", bg='#567', fg='White', command=self.win.destroy).grid(row = 0, column = 0)
        self.yes = tk.Button(text="yes", bg='#567', fg='White', command=self.addItem)
        self.win.mainloop()

    def direction_(self):
        tk.Button(self.win,text='North', bg='#567', fg='White', command = lambda : self.isPathExist([i+j for i, j in zip(self.path, self.directions["north"])], "north")).grid(row = 3, column = 6)
        tk.Button(self.win, text = 'East', bg='#567', fg='White', command = lambda : self.isPathExist([i+j for i, j in zip(self.path, self.directions["east"])], "east")).grid(row = 4, column = 6)
        tk.Button(self.win, text='South', bg='#567', fg='White', command = lambda : self.isPathExist([i+j for i, j in zip(self.path, self.directions["south"])], "south")).grid(row = 5, column = 6)
        tk.Button(self.win, text='West', bg='#567', fg='White', command = lambda : self.isPathExist([i+j for i, j in zip(self.path, self.directions["west"])], "west")).grid(row = 6, column = 6)
    ...
    def checkPathInput(self, userPath):
        """
            To check whether entered path is not empty
        :param userPath: path entered by the user
        :return: True and userpath in case of correct input else False, None
        """
        if len(userPath) != 0:
            userPath = userPath.split()
            return True, userPath
        else:
            print("No path entered! RETRY!\n")
            return False, None
    ...
    def movingMuseum(self):
        if len(self.bag) != self.museumBagThresh:
            if self.map[self.path[0]][self.path[1]] == 1:
                ...
                if len(self.bag) != self.museumBagThresh:
                    self.yes = tk.Button(text = "yes", command = self.addItem, bg='#567', fg='White')
                    self.yes.grid(row = 12, column = 2)
                    self.l1 .grid(row=14, column=1, columnspan=3)
                    self.l2.grid(row=15, column=1, columnspan=3)
    ...
    def addItem(self):
        ...
        if len(self.bag) != self.museumBagThresh:
            if list(self.articleAndDescription[tuple(self.path)].keys())[0] in self.bag:
                ...
                if len(self.bag) == 2:
                    tk.Label(text="Exiting the museum! (in 3 seconds)").grid(row=13, column = 2)
                    self.win.destroy()
        else:
